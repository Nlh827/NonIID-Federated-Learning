...
def partition_data(dataset, datadir, partition, n_nets, alpha, resize=32, augmentation=True, args=None):
    ...
    if args is not None and hasattr(args, "seed"):
        ...
        np.random.seed(seed)
        random.seed(seed)
        logging.info(f"[Reproducibility] partition seed = {seed}")
...
def load_partition_data_distributed_cifar10(process_id, dataset, data_dir, partition_method, partition_alpha,
    ...
    X_train, y_train, X_test, y_test, net_dataidx_map, traindata_cls_counts, \
        cifar10_train_ds, cifar10_test_ds = partition_data(
        dataset,
        data_dir,
        partition_method,
        client_number,
...
def load_partition_data_cifar10(dataset, data_dir, partition_method, partition_alpha, client_number, batch_size,
    ...
    # 对应 R1-C10
    if args is not None and hasattr(args, "result_dir"):
        import os
        import pickle

        os.makedirs(args.result_dir, exist_ok=True)
        partition_path = os.path.join(args.result_dir, "net_dataidx_map.pkl")
        cls_count_path = os.path.join(args.result_dir, "train_cls_counts.pkl")
        ...
        logging.info(f"[Reproducibility] saved partition to {partition_path}")
